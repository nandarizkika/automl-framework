...
class TuningIntegrator:
    ...
    def tune_models(self, 
        ...
        if models_to_tune is not None:
            models = {name: all_models[name] for name in models_to_tune if name in all_models}
            if not models:
                raise ValueError(f"None of the specified models {models_to_tune} found in registry")
        else:
            models = all_models
        ...
        if register_best:
            for name, tuning_result in self.tuner.results.items():
                if 'best_estimator' in tuning_result and tuning_result['best_estimator'] is not None:
                    preprocessor = models[name].preprocessor
                    
                    best_params = tuning_result['best_params']
                    best_score = tuning_result['best_score']
                    
                    logger.info(f"Registering tuned model {name} with score {best_score:.4f}")
