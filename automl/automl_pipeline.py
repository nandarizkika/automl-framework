...
CLASSIFICATION_MODEL_REFERENCE = {
    'RandomForest': {
        ...
        'mitigation_strategies': ['max_depth', 'min_samples_split', 'min_samples_leaf', 'max_features']
    },
    'LogisticRegression': {
    ...
    'GradientBoosting': {
        ...
        'mitigation_strategies': ['learning_rate', 'max_depth', 'min_samples_split', 'n_estimators', 'subsample']
    },
    'DecisionTree': {
        ...
        'mitigation_strategies': ['max_depth', 'min_samples_split', 'min_samples_leaf', 'ccp_alpha']
    },
    'AdaBoost': {
...
class AutoMLPipeline:
    ...
    def __init__(self, problem_type: str = 'classification', random_state: int = 42):
        ...
        self.model_reference = (CLASSIFICATION_MODEL_REFERENCE 
    ...
    def _register_default_models(self):
        ...
        if self.problem_type == 'classification':
            ...
            logger.warning(f"No default models registered for problem type: {self.problem_type}")
    
    def set_overfitting_control(self, 
        ...
        logger.info(f"Overfitting control: detection={'enabled' if detection_enabled else 'disabled'}, "
                   f"auto_mitigation={'enabled' if auto_mitigation else 'disabled'}, "
                   f"threshold={threshold}")
    
    def register_model(self, name: str, model_instance: BaseEstimator, preprocessor: Optional[Preprocessor] = None):
    ...
    def fit(self, X, y):
        ...
        logger.info(f"Completed training {len(successful_models)} models successfully. " +
    ...
    def evaluate(self, X, y):
        ...
        for name, model in models.items():
            try:
                ...
                metrics = self.metrics_calculator.calculate_metrics(y, y_pred, y_pred_proba)
                ...
                if self.X_train is not None and self.y_train is not None:
                    try:
                        ...
                        try:
                            fit_evaluation['improvement_suggestions'] = self.model_evaluator.get_improvement_suggestions(
                                fit_evaluation
                            )
                        except Exception as e:
                            logger.error(f"Error getting improvement suggestions for {name}: {str(e)}")
                            fit_evaluation['improvement_suggestions'] = ["Could not generate suggestions due to an error."]

                        logger.info(f"\nModel Fit Evaluation for {name}:")
                        logger.info(f"Fit Quality: {fit_evaluation.get('fit_quality', 'Unknown')}")
                        logger.info(f"Overfitting Score: {fit_evaluation.get('overfitting_score', 0):.2f}")
                        logger.info(f"Underfitting Score: {fit_evaluation.get('underfitting_score', 0):.2f}")
                    except Exception as e:
                ...
                if self.overfitting_detection_enabled:
                    ...
                    if overfitting_result.get('is_overfitting', False):
                        overfitting_models.append({
                            ...
                            'severity': overfitting_result.get('severity', 'Unknown')
                        })
                        
                        logger.warning(f"Overfitting detected in {name}: "
                        ...
                        if (self.auto_mitigation_enabled and 
                            overfitting_result.get('overfitting_score', 0) >= self.overfitting_threshold):
                            self._apply_overfitting_mitigation(name, model, overfitting_result)
        ...
        if self.results:
            metric_to_optimize = 'f1_score' if self.problem_type == 'classification' else 'r2'
            
            all_models = {**self.results}
            
            best_name = max(all_models, key=lambda x: all_models[x].get(metric_to_optimize, 0))
            ...
            if best_name in self.mitigated_models:
                ...
                logger.info(f"Best model: {self.best_model_name} with {metric_to_optimize}: {all_models[best_name].get(metric_to_optimize, 0):.4f}")
            else:
                self.best_model = self.results[best_name]['model']
                self.best_model_name = best_name
                logger.info(f"Best model: {best_name} with {metric_to_optimize}: {self.results[best_name].get(metric_to_optimize, 0):.4f}")
    ...
    def _apply_overfitting_mitigation(self, model_name, model, overfitting_assessment):
        ...
        if results.get('success', False):
            ...
            logger.info(f"Successfully mitigated overfitting in {model_name} using {strategy.get('name')} strategy")
        else:
            logger.warning(f"Failed to mitigate overfitting in {model_name}: {results.get('message', 'Unknown error')}")
    ...
    def get_model_reference(self, model_name: Optional[str] = None) -> Dict:
        ...
        if model_name is not None:
            ...
            return {name: self.model_reference.get(name, {"description": "Custom or unknown model"}) 
                   for name in registered_models}
    
    def predict(self, X):
        """Make predictions using the best model"""
        if self.best_model is None:
            raise ValueError("No best model available. Call fit() and evaluate() first.")
    ...
    def get_leaderboard(self) -> pd.DataFrame:
        ...
        for name, result in self.results.items():
            row = {
                'model': name,
                'training_time': result['training_log']['training_duration'] if 'training_log' in result else None
            }

            if 'fit_evaluation' in result and 'fit_quality' in result['fit_evaluation']:
                row['fit_quality'] = result['fit_evaluation']['fit_quality']
                row['overfitting_score'] = safe_scalar(result['fit_evaluation'].get('overfitting_score', 0))
                row['underfitting_score'] = safe_scalar(result['fit_evaluation'].get('underfitting_score', 0))

            if 'overfitting_assessment' in result:
                row['overfitting_severity'] = result['overfitting_assessment'].get('severity', 'None')

            if self.problem_type == 'classification':
                if 'train_accuracy' in result.get('fit_evaluation', {}) and 'test_accuracy' in result.get('fit_evaluation', {}):
                    row['train_accuracy'] = safe_scalar(result['fit_evaluation']['train_accuracy'])
                    row['test_accuracy'] = safe_scalar(result['fit_evaluation']['test_accuracy'])
                    row['accuracy_gap'] = safe_scalar(row['train_accuracy'] - row['test_accuracy'])
                else:
                    row['train_accuracy'] = None
                    row['test_accuracy'] = safe_scalar(result.get('accuracy', None))
                    row['accuracy_gap'] = None

                if 'train_precision' in result.get('fit_evaluation', {}) and 'test_precision' in result.get('fit_evaluation', {}):
                    row['train_precision'] = safe_scalar(result['fit_evaluation']['train_precision'])
                    row['test_precision'] = safe_scalar(result['fit_evaluation']['test_precision'])
                    row['precision_gap'] = safe_scalar(row['train_precision'] - row['test_precision'])
                else:
                    row['train_precision'] = None
                    row['test_precision'] = safe_scalar(result.get('precision', None))
                    row['precision_gap'] = None

                if 'train_recall' in result.get('fit_evaluation', {}) and 'test_recall' in result.get('fit_evaluation', {}):
                    row['train_recall'] = safe_scalar(result['fit_evaluation']['train_recall'])
                    row['test_recall'] = safe_scalar(result['fit_evaluation']['test_recall'])
                    row['recall_gap'] = safe_scalar(row['train_recall'] - row['test_recall'])
                else:
                    row['train_recall'] = None
                    row['test_recall'] = safe_scalar(result.get('recall', None))
                    row['recall_gap'] = None

                if 'train_f1' in result.get('fit_evaluation', {}) and 'test_f1' in result.get('fit_evaluation', {}):
                ...
                if 'train_r2' in result.get('fit_evaluation', {}) and 'test_r2' in result.get('fit_evaluation', {}):
                ...
                if 'train_mse' in result.get('fit_evaluation', {}) and 'test_mse' in result.get('fit_evaluation', {}):
                    row['train_mse'] = safe_scalar(result['fit_evaluation']['train_mse'])
                    row['test_mse'] = safe_scalar(result['fit_evaluation']['test_mse'])
                    if row['train_mse'] and row['train_mse'] > 0:
                        row['mse_ratio'] = safe_scalar(row['test_mse'] / row['train_mse'])
                    else:
                        row['mse_ratio'] = None
                else:
                    row['train_mse'] = None
        ...
        for name, result in self.mitigated_models.items():
            ...
            row = {
                ...
                'training_time': result['model'].get_training_log().get('training_duration', None),
                'fit_quality': 'Mitigated',
                'mitigation_strategy': result['strategy']
            }

            if self.problem_type == 'classification':
                row['test_accuracy'] = safe_scalar(mitigated_metrics.get('accuracy', None))
                row['test_precision'] = safe_scalar(mitigated_metrics.get('precision', None))
        ...
        if self.problem_type == 'classification':
            columns = [
                'model', 'training_time', 'fit_quality', 'overfitting_score', 'underfitting_score', 'overfitting_severity',
                'train_accuracy', 'test_accuracy', 'accuracy_gap',
                'train_precision', 'test_precision', 'precision_gap',
                'train_recall', 'test_recall', 'recall_gap',
                'train_f1', 'test_f1', 'f1_gap',
                'roc_auc'
            ]
            ...
            df = df[columns]
            sort_by = 'test_f1' if 'test_f1' in df.columns else 'f1_score' if 'f1_score' in df.columns else 'test_accuracy'
    ...
    def get_fit_evaluation(self, model_name: Optional[str] = None) -> Dict:
        ...
        if model_name is not None:
            ...
            return self.results[self.best_model_name.split('_')[0]].get('fit_evaluation', {})
    ...
    def get_all_fit_evaluations(self) -> Dict[str, Dict]:
        ...
        return {name: result.get('fit_evaluation', {}) for name, result in self.results.items()}
    ...
    def get_all_overfitting_assessments(self) -> Dict[str, Dict]:
        ...
        return {name: result.get('overfitting_assessment', {}) for name, result in self.results.items()}
    ...
    def get_mitigation_strategies(self, model_name: str) -> List[Dict]:
        ...
        overfitting_assessment = self.results[model_name].get('overfitting_assessment', {})
    ...
    def manually_mitigate_overfitting(self, model_name: str, strategy_name: str) -> Dict:
        """
        ...
        if results.get('success', False):
            ...
            self.training_log['mitigated_models'].append(f"{model_name}_{strategy_name}")
            
            logger.info(f"Successfully mitigated overfitting in {model_name} using {strategy_name} strategy")
            ...
            logger.warning(f"Failed to mitigate overfitting in {model_name} using {strategy_name}: {results.get('message', 'Unknown error')}")
    ...
    def get_improvement_suggestions(self, model_name: Optional[str] = None) -> List[str]:
        """
        ...
        if model_name is None:
            model_name = self.best_model_name.split('_')[0] if self.best_model_name else None
        ...
        if model_name and model_name in self.results:
            ...
            if overfitting_assessment.get('is_overfitting', False):
                ...
                all_suggestions = basic_suggestions + ["\nOverfitting Mitigation Strategies:"] + mitigation_suggestions
