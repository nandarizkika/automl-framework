...
class ModelWrapper:
    ...
    def __init__(self, name: str, model_instance: BaseEstimator, preprocessor: Optional[Preprocessor] = None):
        """
    ...
    def fit(self, X, y):
        ...
        logger.info(f"Starting training for {self.name} with {type(self.model).__name__}")
        ...
        try:
            ...
            if self._is_fitted:
                logger.info(f"Successfully trained {self.name} in {training_duration:.2f} seconds")
            else:
                logger.warning(f"Failed to train {self.name} after {training_duration:.2f} seconds")
