...
try:
    ...
    logger.warning("BayesSearchCV not available. Install scikit-optimize for Bayesian optimization.")
...
class HyperparameterTuner:
    ...
    def get_param_grid(self, model_name: str, search_type: str = 'grid') -> Dict:
        ...
        if search_type in ['grid', 'random', 'halving']:
            if 'tree' in model_type and not any(x in model_type for x in ['forest', 'extra', 'boost']):
                return {
                    'max_depth': [3, 5, 7, 10, 15, 20, None],
                    'min_samples_split': [2, 5, 10, 15, 20],
                    'min_samples_leaf': [1, 2, 4, 8],
                    'max_features': ['sqrt', 'log2', None],
                    'criterion': ['gini', 'entropy'] if self.problem_type == 'classification' else ['squared_error', 'friedman_mse', 'absolute_error']
                }
                ...
                return {
                    ...
                    'algorithm': ['SAMME', 'SAMME.R'] if self.problem_type == 'classification' else ['SAMME']
                }
            ...
            elif any(x in model_type for x in ['linear', 'logistic', 'regression', 'ridge', 'lasso']):
                if 'logistic' in model_type or (self.problem_type == 'classification' and 'regression' in model_type):
                    return {
                ...
                logger.warning(f"No specific parameter grid available for {model_name}. Using minimal default grid.")
                return {
                    'random_state': [self.random_state, None]
                }
            ...
            if 'tree' in model_type and not any(x in model_type for x in ['forest', 'extra', 'boost']):
                return {
                    ...
                    'criterion': Categorical(['gini', 'entropy']) if self.problem_type == 'classification' else 
                                Categorical(['squared_error', 'friedman_mse', 'absolute_error'])
                }
                ...
                return {
                    ...
                    'algorithm': Categorical(['SAMME', 'SAMME.R']) if self.problem_type == 'classification' else 
                                Categorical(['SAMME'])
                }
                
            else:
                logger.warning(f"No specific Bayesian parameter space for {model_name}. Using defaults.")
                return None
            ...
            if 'tree' in model_type and not any(x in model_type for x in ['forest', 'extra', 'boost']):
                return {
                    ...
                    'criterion': hp.choice('criterion', 
                                         ['gini', 'entropy'] if self.problem_type == 'classification' else 
                ...
                logger.warning(f"No specific Hyperopt space for {model_name}. Using minimal defaults.")
                return {}
                
        else:
            logger.warning(f"Unsupported search type: {search_type}. Using grid search parameters.")
    ...
    def tune_model(self, 
        ...
        try:
            if search_type == 'grid':
                ...
                def objective(params, model=None, X=None, y=None, cv=None, scoring=None):
                    ...
                    try:
                        scores = cross_val_score(model_copy, X, y, cv=cv, scoring=scoring, n_jobs=self.n_jobs)
                ...
                trial_results = [t['result'] for t in trials.trials if t['result']['status'] == STATUS_OK]
                if trial_results:
                    ...
                    cv_results = {
                        'params': [t['result']['model_params'] for t in trials.trials if 'model_params' in t['result']],
                        'mean_test_score': [t['result']['score'] if 'score' in t['result'] else float('-inf') 
                                          for t in trials.trials],
                        'std_test_score': [np.std(t['result']['cv_scores']) if 'cv_scores' in t['result'] else 0 
                                          for t in trials.trials]
                    }
                else:
                    logger.warning("No successful Hyperopt trials. Falling back to default model.")
                    best_estimator = model_instance
                    best_params = {}
                    best_score = float('-inf')
                    
            else:
                logger.warning(f"Unsupported or unavailable search type: {search_type}. Falling back to grid search.")
                return self.tune_model(model_name, model_instance, X, y, param_grid, 'grid', n_iter, n_splits, refit)
            ...
            logger.info(f"Tuning completed for {model_name} in {tuning_time:.2f} seconds.")
    ...
    def tune_multiple_models(self, 
        ...
        for name, model in models_dict.items():
            ...
            result = self.tune_model(name, model, X, y, search_type=search_type, n_iter=n_iter)
            results[name] = result
    ...
    def get_best_model(self) -> Tuple[str, BaseEstimator, float]:
        ...
        best_model_name = max(self.results, key=lambda name: self.results[name].get('best_score', float('-inf')))
            ...
            if param2 is not None:
                ...
                if param1_numeric and param2_numeric:
                    ...
                    plt.scatter(
                        x=params[param1],
                        y=scores,
                        c=params[param2].astype('category').cat.codes if not param2_numeric else params[param2],
                        cmap='viridis',
                        alpha=0.7
                    )
                    ...
                    if not param2_numeric:
                        ...
                        for i, cat in enumerate(categories):
                            handles.append(plt.scatter([], [], c=[i], cmap='viridis', label=cat))
        ...
        try:
            ...
            for param in param_names:
                if not pd.api.types.is_numeric_dtype(params_df[param]):
                    try:
                        params_df[param] = pd.to_numeric(params_df[param], errors='raise')
                    except:
                        continue
    ...
    def save_tuning_results(self, 
        ...
        if model_name is not None:
            ...
            if 'best_estimator' in result and result['best_estimator'] is not None:
                ...
                logger.info(f"Best estimator for {model_name} saved to {estimator_path}")
            ...
            try:
                importance_path = os.path.join(directory, f"{model_name}_parameter_importance.png")
                self.plot_parameter_importance(model_name, output_file=importance_path)
                
                cv_results = result.get('cv_results', {})
                if 'params' in cv_results and cv_results['params'] and len(cv_results['params'][0]) >= 2:
                    param_names = list(cv_results['params'][0].keys())
                    tuning_path = os.path.join(directory, f"{model_name}_tuning_plot.png")
                    self.plot_tuning_results(model_name, param_names[0], param_names[1], tuning_path)
            except Exception as e:
