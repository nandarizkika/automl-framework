...
class TestHyperparameterTuner:
    ...
    def test_parameter_grid_regression(self):
        ...
        dt_params = tuner.get_param_grid('DecisionTree', 'grid')
        assert 'criterion' in dt_params
        assert any('squared_error' in str(criterion) or 'friedman_mse' in str(criterion) 
    ...
    def test_parameter_importance_plotting(self, small_classification_data, temp_directory):
    ...
    def test_save_load_tuning_results(self, small_classification_data, temp_directory):
        ...
        if 'error' not in result:
            ...
            results_file = os.path.join(temp_directory, 'DecisionTree_tuning_results.pkl')
            estimator_file = os.path.join(temp_directory, 'DecisionTree_best_estimator.pkl')
            
            assert os.path.exists(results_file)
            assert os.path.exists(estimator_file)
            
            new_tuner = HyperparameterTuner(problem_type='classification')
            loaded_results = new_tuner.load_tuning_results(temp_directory, 'DecisionTree')
    ...
    def test_save_all_tuning_results(self, small_classification_data, temp_directory):
        ...
        for model_name in models.keys():
            results_file = os.path.join(temp_directory, f'{model_name}_tuning_results.pkl')
