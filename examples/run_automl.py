...
def display_enhanced_leaderboard(leaderboard, problem_type='classification'):
    ...
    info_columns = ['model', 'training_time', 'fit_quality', 'overfitting_score', 
                    'underfitting_score', 'overfitting_severity']
    ...
    for group_name, columns in metric_groups.items():
        if any(col in display_df.columns for col in columns):
            ...
            if existing_columns:
                try:
                    print(f"\n{group_name} Metrics:")
                    print(display_df[['model'] + existing_columns].to_string(index=True))
                except Exception as e:
...
def run_automl(
    ...
    for key, value in data_report.items():
        ...
        elif key == 'zero_variance_features' and isinstance(value, list) and len(value) > 10:
    ...
    if handle_overfitting and 'mitigated_models' in training_summary:
        print(f"- Models mitigated for overfitting: {len(training_summary['mitigated_models'])}")
    ...
    leaderboard = None
    try:
        if evaluation_succeeded and hasattr(automl.pipeline, 'results') and automl.pipeline.results:
            leaderboard = automl.get_leaderboard()
            display_enhanced_leaderboard(leaderboard, problem_type)
            
            mitigated_models = []
            if leaderboard is not None:
                mitigated_models = [model for model in leaderboard['model'] if str(model).find('mitigated') != -1]
            if mitigated_models:
    ...
    try:
        ...
        if handle_overfitting and hasattr(automl.pipeline, 'get_all_overfitting_assessments'):
        ...
        for model_name, eval_data in all_evaluations.items():
            ...
            if fit_quality == 'Good fit' and not (model_name in overfitting_assessments and overfitting_assessments[model_name].get('is_overfitting', False)):
                continue
                
            print(f"\n{model_name}:")
            print(f"- Fit Quality: {fit_quality}")
            print(f"- Overfitting Score: {format_metric(eval_data.get('overfitting_score', 0))}")
            print(f"- Underfitting Score: {format_metric(eval_data.get('underfitting_score', 0))}")
            ...
            if model_name in overfitting_assessments:
                ...
                if assessment.get('is_overfitting', False):
                    print(f"- Overfitting Severity: {assessment.get('severity', 'Unknown')}")
                    
                    details = assessment.get('details', {})
                    if details:
                        if 'accuracy_gap' in details:
                            print(f"  * Accuracy Gap: {format_metric(details['accuracy_gap'])}")
                        if 'precision_gap' in details:
                            print(f"  * Precision Gap: {format_metric(details['precision_gap'])}")
                        if 'recall_gap' in details:
                            print(f"  * Recall Gap: {format_metric(details['recall_gap'])}")
        ...
        if handle_overfitting and hasattr(automl.pipeline, 'get_mitigated_models'):
            ...
            if mitigated_models_info:
                ...
                for name, info in mitigated_models_info.items():
                    print(f"- {name}: Applied {info['strategy']} strategy to {info['original_model']}")
    except Exception as e:
        ...
        try:
            ...
            if problem_type == 'classification':
                ...
                try:
                    ...
                    print(f"- Accuracy Gap (Train-Test): {format_metric(train_accuracy - accuracy)}")
    ...
    try:
        ...
        if feature_importance and ('feature_importances_' in feature_importance or 'coefficients' in feature_importance):
    ...
    if visualization and handle_overfitting:
        ...
        try:
            ...
            for model_name, eval_data in all_evaluations.items():
                if problem_type == 'classification':
                    if not all(k in eval_data for k in ['train_accuracy', 'test_accuracy']):
                        continue
                else:
                ...
                if hasattr(train_score, '__len__') and not isinstance(train_score, (str, dict)) and len(train_score) > 1:
                    train_score = float(np.mean(train_score))
                if hasattr(test_score, '__len__') and not isinstance(test_score, (str, dict)) and len(test_score) > 1:
            ...
            if model_data:
                ...
                plot_df.plot(x='Model', y=['Train Score', 'Test Score'], kind='bar', ax=ax1)
                ax1.set_title('Train vs Test Performance')
                ax1.set_ylabel('Score')
                ax1.set_xticklabels(ax1.get_xticklabels(), rotation=45, ha='right')
                
                plot_df.plot(x='Model', y='Overfitting Score', kind='bar', ax=ax2, color='orange')
                ...
                print(f"Saved overfitting visualization to: {os.path.join(output_dir, 'overfitting_analysis.png')}")
        except Exception as e:
