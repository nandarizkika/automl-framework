...
try:
    ...
    warnings.warn("TuningIntegrator not available. Hyperparameter tuning examples will be skipped.")
...
def hyperparameter_tuning_demo(automl, X_train, y_train):
    ...
    models_to_tune = ['RandomForest', 'GradientBoosting_Regularized', 'ExtraTrees_Custom']
...
def comprehensive_model_analysis(automl, X_test, y_test):
    ...
    try:
        ...
        if not leaderboard.empty:
            ...
            display_cols = ['model', 'test_accuracy', 'test_f1', 'fit_quality', 'training_time']
            ...
            if 'test_accuracy' in leaderboard.columns:
                best_model = leaderboard.loc[leaderboard['test_accuracy'].idxmax(), 'model']
                worst_model = leaderboard.loc[leaderboard['test_accuracy'].idxmin(), 'model']
    ...
    try:
        ...
        if importance:
            ...
            if 'feature_importances_' in importance:
                ...
                print(f"  Top 5 features contribute: {importances.nlargest(5).sum():.2%}")
    ...
    try:
        ...
        try:
            ...
            print(f"  Predictions with >90% confidence: {(confidence_scores > 0.9).sum()}/{len(confidence_scores)}")
...
def create_ensemble_model(automl, X_train, y_train):
    ...
    try:
        ...
        print("Note: Full ensemble implementation requires model extraction from pipeline")
...
def visualization_demo(automl, X_test, y_test):
    ...
    try:
        ...
        fig.suptitle('AutoML Framework - Advanced Analysis', fontsize=16, fontweight='bold')
        ...
        try:
            ...
            if fit_evaluations:
                ...
                colors = ['red' if score > 0.3 else 'orange' if score > 0.1 else 'green' 
                         for score in overfitting_scores]
                
                bars = ax2.bar(range(len(model_names)), overfitting_scores, color=colors, alpha=0.7)
                ...
                ax2.axhline(y=0.1, color='orange', linestyle='--', alpha=0.7, label='Slight')
                ax2.axhline(y=0.3, color='red', linestyle='--', alpha=0.7, label='Moderate')
                ax2.legend()
        except Exception:
            ax2.text(0.5, 0.5, 'Overfitting analysis\nnot available', 
        ...
        try:
            ...
            if importance and 'feature_importances_' in importance:
                ...
                bars = ax3.barh(range(len(top_features)), top_importances, color='lightcoral', alpha=0.7)
